...
import com.fangelo.kotlinrpg.ui.screen.SettingsScreen;
...
public class ScreenManager {
	...
	static private void updateDebug(float delta) {
		...
		if (statsUpdateTime > 1.0f) {
			...
			if (GLProfilerSingleton.INSTANCE.getProfiler().isEnabled()) {
				statsBuilder.append(" dc: ");
				statsBuilder.append(GLProfilerSingleton.INSTANCE.getProfiler().getDrawCalls() / statsUpdateTimeFrames);
				statsBuilder.append(" vc: ");
				statsBuilder.append((int) (GLProfilerSingleton.INSTANCE.getProfiler().getVertexCount().total / statsUpdateTimeFrames));
				GLProfilerSingleton.INSTANCE.getProfiler().reset();
			}
